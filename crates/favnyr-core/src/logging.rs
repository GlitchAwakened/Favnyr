...
//! `cache/favnyr.log`. The file is essential in release builds: the app then
...
//!
//! Format driven by `FAVNYR_LOG_FORMAT`: `json` | `pretty` | other (compact).
...
pub fn init() {
    ...
    let format = std::env::var("FAVNYR_LOG_FORMAT").unwrap_or_default();
    ...
    if let Err(err) = res {
        eprintln!("[favnyr] tracing init failed: {err}");
    }
}
