fn main() {
    ...
    // Windows: embeds the app icon into favnyr.exe (Explorer, taskbar, alt-tab).
    // Non-blocking — a tooling failure (rc.exe missing) must not break the
    // build; the exe then ships without an embedded icon.
    #[cfg(windows)]
    {
        println!("cargo:rerun-if-changed=assets/favnyr.ico");
        let mut res = winresource::WindowsResource::new();
        res.set_icon("assets/favnyr.ico");
        if let Err(e) = res.compile() {
            println!("cargo:warning=failed to embed the icon (favnyr.ico): {e}");
        }
    }
}
