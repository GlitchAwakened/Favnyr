...
//! `DataTransferManager` for its window first — Explorer does, Favnyr did not.
//! So Favnyr drives the sheet itself: register the manager, provide the selected
...
pub fn share_files(hwnd: isize, paths: &[PathBuf]) -> Result<()> {
    ...
    let title = HSTRING::from(
        paths
            ...
            .unwrap_or_else(|| "Favnyr".to_string()),
    );
